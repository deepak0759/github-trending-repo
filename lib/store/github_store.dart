...
abstract class _GitStore with Store {
  ...
  StoreState get mystate {
    if (_repofuture == null || _repofuture.status == FutureStatus.rejected) {
  ...
  Future<void> getRepoList(int days) async {
    ...
    try {
      _repofuture = ObservableFuture(_gitHubService.getList(
          'created:>$formattedDate', 'stars', 'desc', '0', '50'));
  ...
  Future<void> search(var query) async {
    try {
      _searchFuture = ObservableFuture(
          _gitHubService.getList(query, 'stars', 'desc', 'start', '50'));
